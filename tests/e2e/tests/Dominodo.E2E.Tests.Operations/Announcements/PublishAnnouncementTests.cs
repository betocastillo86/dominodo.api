...
/// <summary>
/// Black-box coverage for <c>PUT /api/v1/announcements/{id}/publish</c> (AnnouncementsController.Publish),
/// guarded by <c>[HasPermission(Permissions.AnnouncementsEdit)]</c> and scoped by the <c>X-Tenant</c> header.
/// Draft → Published. Success is 204 NoContent. No request body/validator, so there is no 400 Validation.Failed;
/// an unknown id ⇒ 404. Authorization is proven on both branches (Platform + Tenant grant) plus tenant isolation.
/// </summary>
[TestFixture]
public sealed class PublishAnnouncementTests : BaseOperationsTests
{
    ...
    {
        // Act — no token
        var response = await OperationsClient.PublishAnnouncement(Guid.NewGuid(), tenant: SeededTenantSlug);
    ...
    {
        // Arrange — a bearer carrying announcements.view (not announcements.edit): publish is an edit operation.
        var token = JwtTokenFactory.GenerateToken(DominodoConstants.Permission.AnnouncementsView);

        // Act
        var response = await OperationsClient.PublishAnnouncement(
    ...
    [Test]
    public async Task _204_WhenUserHasAnnouncementsEditOnPlatform_AndTransitionsToPublished()
    {
        // Arrange — a fresh draft, published by a Platform announcements.edit holder (cross-tenant grant).
        var draft = await OperationsRequestBuilder.CreateAnnouncementAsync(tenantSlug: SeededTenantSlug);
        var token = JwtTokenFactory.GenerateToken(DominodoConstants.Permission.AnnouncementsEdit);

        // Act
        var response = await OperationsClient.PublishAnnouncement(draft.Id, tenant: SeededTenantSlug, token: token);

        // Assert
        response.StatusCode.ShouldBe(HttpStatusCode.NoContent);

        // Verify the transition: Draft → Published, with a publish timestamp and publisher stamped.
        var published = await OperationsRequestBuilder.GetAnnouncementAsync(SeededTenantSlug, draft.Id);
        published.Status.ShouldBe("Published");
        published.PublishedAtUtc.ShouldNotBeNull();
        published.PublishedByUserId.ShouldNotBeNull();
    }

    [Test]
    public async Task _204_WhenUserHasAnnouncementsEditInTenant()
    {
        // Arrange — a draft in the seeded tenant, published by the seeded tenant user (tenant branch).
        var draft = await OperationsRequestBuilder.CreateAnnouncementAsync(tenantSlug: SeededTenantSlug);
        var token = JwtTokenFactory.GenerateTenantToken(DominodoConstants.Permission.AnnouncementsEdit);
