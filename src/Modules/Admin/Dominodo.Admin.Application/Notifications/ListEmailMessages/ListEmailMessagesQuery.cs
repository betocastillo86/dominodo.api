...
internal sealed record ListEmailMessagesQuery(DeliveryStatus? Status = null, int Page = 1, int PageSize = 20) : IQuery<PagedResult<EmailMessageDto>>;
    ...
    : IQueryHandler<ListEmailMessagesQuery, PagedResult<EmailMessageDto>>
{
    public async Task<Result<PagedResult<EmailMessageDto>>> Handle(ListEmailMessagesQuery query, CancellationToken ct)
    {
        ...
        var (items, total) = await messages.ListAsync(tenantId, query.Status, page, ct);
